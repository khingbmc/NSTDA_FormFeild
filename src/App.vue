  ...
  if ("undefined" == typeof window.powerbiresizescript) {
    ...
    window.onmessage = function(event) {
      var isReportPageLoadedEvent = function(event) {
        try {
          if (
            event &&
            event.data &&
            event.data.url === "/reports/undefined/events/pageChanged"
          ) {
            return !0;
          }
        } catch (error) {
      ...
      if (isReportPageLoadedEvent(event)) {
        ...
        }, (iframe.parentNode.getAttribute("pbi-resize-delay-show") || 1) *
          1000);
      }
    };
    function getChildByTag(parent, tagName) {
      if (parent) {
        for (var i = 0; i < parent.children.length; i++) {
          if (
            parent.children[i].tagName.toLowerCase() === tagName.toLowerCase()
          ) {
    ...
    var e = function() {
      for (
        var e = document.querySelectorAll('[pbi-resize="powerbi"]'), i = 0;
        i < e.length;
        i++
      ) {
        e[i].style.width = "100%";
        ...
        var newSrc = !(webSrc && mobileSrc)
          ? webSrc
          : isWebSize
          ? webSrc
          : mobileSrc;
        var resizedToWeb =
          ((iframe && iframe.src == mobileSrc) ||
            (img && img.src == mobileImg)) &&
          isWebSize &&
          mobileSrc != webSrc;
        var resizedToMobile =
          ((iframe && iframe.src == webSrc) || (img && img.src == webImg)) &&
          !isWebSize &&
          mobileSrc != webSrc;
        ...
        if (!currentSrc) {
          ...
          if (img) {
            img.setAttribute(
              "src",
              !isWebSize && mobileImg ? mobileImg : webImg
            );
          }
          if (
            (!webImg && webSrc && isWebSize) ||
            (!mobileImg && mobileSrc && !isWebSize)
          ) {
            iframe.setAttribute(
              "src",
              !isWebSize && mobileSrc ? mobileSrc : webSrc
            );
            showElement(iframe);
            resize();
            break;
            ...
            switch (loadEvent) {
              ...
              case "seconds-timeout":
                var timeout =
                  parseInt(e[i].getAttribute("pbi-resize-seconds")) * 1000;
        ...
        if (
          (currentSrc == webImg && !webImg && webSrc && isWebSize) ||
          (currentSrc == mobileImg && !mobileImg && mobileSrc && !isWebSize)
        ) {
          showElement(iframe);
          ...
          changeCurrentSrc(
            e[i].children[0],
            isWebSize,
            currentSrcIsImage ? webImg : webSrc,
            currentSrcIsImage ? mobileImg : mobileSrc,
            newSrc
          );
        }
        if (
          currentSrcIsImage &&
          ((resizedToMobile && !mobileImg && mobileSrc) ||
            (resizedToWeb && !webImg && webSrc))
        ) {
          showElement(iframe);
        } else if (
          !currentSrcIsImage &&
    ...
    function resizeElement(
      ...
      if (actualWidth < 569 && pageSize === p169) {
        ...
        actualWidth < 320 ||
        actualWidth / pageSize < 320 ||
        (pageHeight < 320 && pageSize !== p169 && pageSize !== p43)
      ) {
        ...
        element.style.height =
          Math.max(element.clientWidth / pageSize, 320) + heightOffset + "px";
      }
      if (warn) {
        console.warn(
          "pbi-resize: requested iframe dimension is below the minimum supported dimensions. Minimum supported width is 320px. Minimum supported height is 376px. Change your Power BI report page size to ensure your content looks great when embedded in your web page or blog."
        );
      }
    }
